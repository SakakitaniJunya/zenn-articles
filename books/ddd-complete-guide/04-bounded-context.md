...
**問題**: 次の要件を持つオンライン学習プラットフォームの Bounded Context を設計してください。

- 受講者が講座を購入できる
- 講師が講座を作成・公開できる
- 受講者が動画を再生でき、視聴完了率が記録される
- 受講者が修了証を取得できる（80%以上完了で発行）
- 管理者が受講者の学習状況を分析できる
- 講師へのロイヤリティ支払いを月次で行う

**解答**:

まず言語の変化点を特定します。「講座」はコンテキストによって意味が異なります。「コンテンツ管理の講座」は動画ファイルとカリキュラム構造ですが、「受講管理の講座」は購入した受講資格です。「修了」は学習コンテキストでは「80%視聴」ですが、請求コンテキストには存在しません。

```mermaid
graph LR
    subgraph CourseCtx["Course Context（講座管理）"]
        CO[Course]
        LEC[Lecture]
        INS[InstructorProfile]
    end

    subgraph EnrollCtx["Enrollment Context（受講管理）"]
        EN[Enrollment]
        STU[Learner]
        PUR[Purchase]
    end

    subgraph LearningCtx["Learning Context（学習進捗）"]
        PR[LearningProgress]
        VP[VideoWatchRecord]
        CERT[Certificate]
    end

    subgraph BillingCtx["Billing Context（精算）"]
        ROYAL[RoyaltyCalculation]
        PAY[RoyaltyPayment]
    end

    subgraph AnalyticsCtx["Analytics Context（分析）"]
        REP[LearningReport]
        DASH[InstructorDashboard]
    end

    CourseCtx -->|"CoursePublished (event)"| EnrollCtx
    EnrollCtx -->|"EnrollmentCreated (event)"| LearningCtx
    LearningCtx -->|"LectureCompleted (event)"| AnalyticsCtx
    LearningCtx -->|"CourseCompleted (event)"| BillingCtx
    EnrollCtx -->|"PurchaseCompleted (event)"| BillingCtx
```

**設計根拠の要点**:
- CourseContext: 講師視点での「何を教えるか」の管理。変更理由は「カリキュラムの改定」
- EnrollmentContext: 受講者の購入・受講資格管理。変更理由は「購入フローの変更・価格改定」
- LearningContext: 実際の学習行動と進捗。変更理由は「修了基準の変更・証明書フォーマット」
- BillingContext: 講師へのロイヤリティ計算。変更理由は「収益分配ルールの変更」
- AnalyticsContext: 管理者向け分析。変更理由は「レポート要件の変更」

LearningContext に「修了証（Certificate）」が属する理由は、修了の判断ロジック（80%以上視聴という計算）がこのコンテキストに存在するためです。BillingContext に修了証を持たせると、学習進捗の詳細を請求コンテキストが知る必要が生じます。

### 問2: 間違ったBC設計を修正する

**問題**: 次の設計の問題点を3つ以上指摘し、修正案を提示してください。

```csharp
namespace HospitalSystem.SharedContext
{
    class Patient
    {
        Guid Id;
        ...
        List<BedAssignment> BedAssignments;
...
1. **神クラス（God Class）**: `Patient` が診療・処方・請求・病床管理・保険・緊急連絡の6つの関心事を持っています。変更の理由が全く異なる属性が1クラスに集まっています。

2. **コンテキスト境界違反**: 診療コンテキストの `MedicalRecordNumber` と請求コンテキストの `Invoice` が同一クラスに存在します。請求ロジックの変更が診療記録クラスに影響します。

3. **言語の曖昧さ**: `Patient` は診療の場合は「治療を受けている人」であり、請求の場合は「代金を支払う法的主体」であり、病棟管理の場合は「ベッドを占有している人」です。3つの意味が混在しています。

4. **整合性要件の混在**: 診察予約と請求書の支払い状況は、整合性要件が全く異なります（前者は診察スケジュールのトランザクション、後者は会計サイクルのトランザクション）。

修正案:

```csharp
// 診療コンテキスト — 医療行為に関する情報に特化
namespace ClinicalContext.Domain
{
    class ClinicalPatient
    {
        ...
        List<Appointment> Appointments;
        List<Prescription> Prescriptions;
        // Invoices・Insurance・PaymentHistory は存在しない
    }
}

// 請求コンテキスト — 金銭の授受に関する情報に特化
namespace BillingContext.Domain
{
    class BillingPatient
    {
        PatientId PatientId;      // IDのみ（ClinicalPatientへの依存なし）
        InsuranceCoverage Insurance;
        PaymentHistory PaymentHistory;
        List<Invoice> Invoices;
        // MedicalRecordNumber・BloodType・Prescriptions は存在しない
    }
}

// 病棟管理コンテキスト — 物理的なベッド管理に特化
namespace WardContext.Domain
{
    class AdmittedPatient
    {
        PatientId PatientId;      // IDのみ
        List<BedAssignment> BedAssignments;
        EmergencyContact EmergencyContact;
        // Prescriptions・Invoices は存在しない
    }
}
```

コンテキスト間の連携は統合イベントで行います。診察が完了したら `AppointmentCompletedIntegrationEvent` を発行し、BillingContext がそれを受け取って請求レコードを作成します。

### 問3: コードレビューでBCの問題を指摘する

**問題**: 次のPRのコードに対して、コードレビューコメントを書いてください。

```csharp
// PR: 注文コンテキストに「在庫自動補充」機能を追加
// OrderContext.Application/Services/OrderService.cs
public sealed class OrderService(
    IOrderRepository orderRepo,
    IProductRepository productRepo,    // ← InventoryContextのリポジトリ
    ISupplierRepository supplierRepo)  // ← PurchasingContextのリポジトリ
{
    public async Task PlaceOrderAsync(PlaceOrderCommand cmd)
    {
        ...
        // 注文後に在庫を確認して、少なければ自動補充
        var product = await productRepo.GetAsync(cmd.ProductId);
        if (product.Stock < product.ReorderPoint)
        {
            var supplier = await supplierRepo.GetPreferredAsync(cmd.ProductId);
            await supplier.SendReplenishmentOrderAsync(product, product.ReorderQuantity);
        }
    }
}
```
...
> **[BC境界違反・責務分散] OrderContext が InventoryContext と PurchasingContext に直接依存しています**
>
> このPRには3つの設計上の問題があります。
>
> **問題1 — コンテキスト境界違反**: `IProductRepository`（在庫コンテキスト）と `ISupplierRepository`（購買コンテキスト）を OrderContext のサービスが直接依存しています。「注文処理」という1つのユースケースが3つのコンテキストに直接アクセスしており、境界が無意味になっています。
>
> **問題2 — 責務の混在**: 「注文処理」「在庫補充判断」「仕入先への発注」は3つの異なる業務責務であり、同一ユースケースに押し込むべきではありません。変更の理由が全く異なります（注文フローの変更 / 在庫補充ルールの変更 / サプライヤー管理の変更）。
>
> **問題3 — 同期的な外部依存**: `supplier.SendReplenishmentOrderAsync` は外部システムへの同期呼び出しです。注文処理のレスポンスタイムが仕入先システムの応答時間に依存し、仕入先システムの障害が注文処理の障害になります。
>
> **修正案**: `OrderPlaced` 統合イベントを発行するだけに留め、InventoryContext がそれを受け取って在庫チェックを行い、必要であれば `ReplenishmentRequired` 統合イベントを発行し、PurchasingContext がそれを受け取って仕入先への発注を行う設計にしてください。OrderContext は在庫や仕入れの詳細を一切知る必要がなくなり、各コンテキストが独立して動けるようになります。

---

## 参考文献と著者の解釈

### Evans (2003) ch.14, ch.15 の内容と著者の解釈

Eric Evans の「Domain-Driven Design: Tackling Complexity in the Heart of Software」（2003年）は、第14章「Maintaining Model Integrity」でコンテキストの境界を、第15章「Distillation」でモデルの本質を蒸留することを論じています。

Evansが第14章で示した Context Map は、Bounded Context 間の関係パターンを定義したものです。Shared Kernel（共有カーネル）・Customer-Supplier（顧客-供給者）・Conformist（適合者）・Anti-Corruption Layer（腐敗防止層）・Separate Ways（独立した道）という5つのパターンは、コンテキストがどのように共存するかの選択肢を示しています。

著者の解釈として、Evansが最も重要視したのは「境界を引くこと」よりも「境界を明示すること」です。暗黙の境界は最悪であり、たとえ間違った境界であっても明示することで「この境界は間違っている」という議論が可能になります。境界の明示がチームの共通認識を作る第一歩です。

### Vaughn Vernon (2013) の見解

Vaughn Vernon の「Implementing Domain-Driven Design」（2013年）は、Evansの概念をより実装寄りに詳述しています。Vernon は特に「Context Map を明示的に文書化し、チーム間で共有すること」の重要性を強調しています。Vernon の見解では、コンテキスト間の関係パターンの選択（特に上流・下流の関係）がシステムの進化に大きな影響を与えます。

Vernon の重要な警告として「Bounded Contextを小さく保つことへの誘惑に注意せよ」があります。マイクロサービスブームの影響で、1クラス1サービスのような過度な分割が起きることがあります。正しいBounded Contextは「言語の境界」で決まるのであり、技術的な分割容易性で決めるべきではないと彼は強調します。

### Alberto Brandolini と EventStorming の補足

Alberto Brandolini は2013年頃に EventStorming を考案しました。ドメインイベントを付箋に書き出し、時系列に並べながらドメインエキスパートとエンジニアが共同でシステムを探索する手法です。Bounded Context の境界を事前設計ではなく「発見」によって導出するという思想は、本章で説明した「言語の変化点」の発見と深く一致しています。

Brandolini はこう述べています。「境界は事前に設計するものではない。ドメインイベントを発見することを通して、自然に現れてくるものだ。」この言葉は、アーキテクトが設計室で孤独に境界を決めるのではなく、ドメインエキスパートとの対話の中から境界が浮かび上がることを示しています。

EventStormingのプロセスで境界を発見するコツとして、付箋の「色」に注意することが挙げられます。同じイベントが複数の部署で異なる反応（ポリシー）を引き起こす場合、その2つのポリシーは異なるコンテキストに属している可能性が高いです。「注文確定」というイベントに対して、倉庫チームが「ピッキングリストを作る」と反応し、経理チームが「請求書の予約を作る」と反応する場合、この2つの反応は別のコンテキストで処理されるべきです。

### 著者の実務経験からの補足

実際のプロジェクトを通じて感じる最も重要な教訓は「Bounded Context の設計は1回で完成しない」ということです。業務の理解が深まるにつれて、境界は見直されるべきです。本章のECサイトの例でも、CatalogContextは実装後3ヶ月に追加されました。

初期段階では「大きすぎる境界」から始めて、業務の理解が深まるにつれて分割していくアプローチが、多くのプロジェクトで現実的です。最初から完璧な境界を設計しようとすることは、まだ十分に理解できていない業務に対して過度にコミットすることを意味し、後の変更コストが高くなります。

また、境界の妥当性を評価する最も実践的な方法は「この境界を前提に、チームが独立して動けるか？」という問いを定期的に立てることです。調整コストが高すぎると感じたら、境界の見直しを検討します。Bounded Context は生き物であり、組織と業務の進化とともに継続的に磨かれていくものです。

もう一つ重要な実務からの教訓は「境界の議論はエンジニアだけで行ってはいけない」ということです。Bounded Context の境界はドメインエキスパートとの対話から生まれるものであり、エンジニアがホワイトボードの前だけで決めた境界はほぼ確実に後から修正が必要になります。ドメインエキスパートを境界設計の場に巻き込むことが、長持ちする境界設計の秘訣です。また、境界の設計は一度行えば終わりではなく、四半期ごと・あるいは大きな業務変化があったタイミングで見直す習慣を組織として持つことが理想的です。そのためにも Context Map を生きたドキュメントとして維持し、変更のたびに更新する文化を根付かせることが肝要です。

---

*本章は第5章「Context Map — 境界間の関係を設計する」へと続きます。*
